...
import rospy
from OwnYolo.object_detection import ObjectDetection
try:
...
class ImageRecognition(threading.Thread):
    ...
    def __init__(self, queue):
        ...
        self.detectobjects = ObjectDetection()
    ...
    def run(self):
        ...
        while True:
            ...
            self.image = self.drawboundingboxes(self.image)

            cv2.imshow("frame", self.image)
            self.robotcontrol.MoveRobot(0, 0, 0.1)
...
#        # Return processed frames
    ...
    def drawboundingboxes(self, img):
        (class_ids, scores, boxes) = self.detectobjects.detect(img)
        detect_images = []
        ...
        for box in boxes:
            ...
            #detect_images = detect_images.append(cutout)
            '''    
